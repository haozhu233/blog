...
<div style="display: inline-block;vertical-align:top; width: 50%;"> test</div>
Now let's try to recreate the input-distance to covariance figure  using the functions we defined here. Our result is the same as the figures (e.g. Figure 4.1) in the R&W textbook. 
...
sqexp_kernel_func <- function(r, l = 1) {
  exp(-0.5 * (r/l)^2)
}

matern_kernel_func <- function(r, l = 1, v = 1) {
  r <- abs(r)
  r[r == 0] <- 1e-8
  part1 = 2^(1-v) / gamma(v)
...
```{r, echo = F}
r = seq(0.00001, 3, length = 30)
sqexp_plot <- plot_ly(type = 'scatter', mode = 'lines') %>%
  add_trace(x = r, y = sqexp_kernel_func(r, 0.5), name = "l=0.5") %>%
  add_trace(x = r, y = sqexp_kernel_func(r, 2), name = "l=2") %>%
  add_trace(x = r, y = sqexp_kernel_func(r, 10), name = "l=10") 
matern_plot <- plot_ly(type = 'scatter', mode = 'lines') %>%
  add_trace(x = r, y = matern_kernel_func(r, v=0.5), name = "l=1,v=0.5") %>%
  add_trace(x = r, y = matern_kernel_func(r, v=2), name = "l=1,v=2") %>%
  add_trace(x = r, y = matern_kernel_func(r, v=10), name = "l=1,v=10") 
subplot(sqexp_plot, matern_plot)
```
