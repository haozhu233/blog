...
author: Hao Zhu
date: "2019-09-12 (Updated on `r Sys.Date()`)"
...
In this assignment, we are implementing two popular kernels, namely the Squared Exponential kernel and the Matern kernel. 
...
```{python, echo = F, fig.margin = TRUE, fig.cap = "Covariance function for the Squared Exponential kernel", results = F}
...
plt.plot(sqexp_x2, sqexp_y_half, 'b', label = "l=0.5")
plt.plot(sqexp_x2, sqexp_y_2, 'r:', label = "l=2")
plt.plot(sqexp_x2, sqexp_y_10, 'g:', label = "l=10")
...
The Matern kernel (Eq. 4.14) is defined as below:
```{python}
...
```{python, echo = F, fig.margin = TRUE, fig.cap = "Reproducing Figure 4.1 for the Matern kernel", results = F}
...
plt.plot(matern_x2, matern_y_half, 'b', label = "v=1/2")
plt.plot(matern_x2, matern_y_2, 'r:', label = "v=2")
plt.plot(matern_x2, matern_y_10, 'g:', label = "v=10")
plt.legend(loc = "upper right")
plt.xlabel("input distance, r")
plt.ylabel("covariance, k(r)")
plt.show()
...
I also tried to recreate Figure 4.1 of the R&W book using the function we defined here. The result, shown in Figure 2, is pretty close. 
